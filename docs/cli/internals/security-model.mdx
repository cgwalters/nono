...
This layer provides a strong sandbox for file content access. A process restricted by Landlock cannot read or write paths outside its allowed set. The second layer exists to make the sandbox more usable, not more secure.

Note that Landlock does not mediate all file-related syscalls. Operations like `chmod(2)`, `chown(2)`, and `setxattr(2)` are not currently restricted by Landlock — a sandboxed process can change metadata on paths it can name by path, even without read or write access to those paths. See [Landlock limitations](/cli/internals/landlock#file-metadata-operations-chmod-chown-setxattr-utime) for details.
